...
class AffinityPolicyTest(xds_k8s_testcase.RegularXdsKubernetesTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_affinity(self) -> None:  # pylint: disable=too-many-statements
        ...
        with self.subTest("01_create_session_affinity_policy"):
            self.td.create_session_affinity_policy()

        with self.subTest("02_create_backend_services"):
        ...
        with self.subTest("06_start_test_servers"):
            test_servers = self.startTestServers(replica_count=_REPLICA_COUNT)

        with self.subTest("07_add_server_backends_to_backend_services"):
        ...
        with self.subTest("11_first_100_affinity_rpcs_pick_same_backend"):
        ...
        with self.subTest("12_turn_down_server_in_use"):
            for server in test_servers:
                if server.hostname == first_backend_inuse:
                    server.set_not_serving()

        with self.subTest("13_wait_for_unhealth_status_propagation"):
        ...
        with self.subTest("14_next_100_affinity_rpcs_pick_different_backend"):
            rpc_stats = self.getClientRpcStats(test_client, _RPC_COUNT)
            json_lb_stats = json_format.MessageToDict(rpc_stats)
            rpc_distribution = xds_url_map_testcase.RpcDistributionStats(
                json_lb_stats
